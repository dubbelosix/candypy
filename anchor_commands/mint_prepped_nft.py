...
async def mint_prepped_nft(async_client, user_key, config_pub_key_string, mint_key, treasury_key_str, approval_instruction, transfer_authority_keypair):
    ...
    result = await candyprog.rpc["mint_nft"](
        ctx=Context(
            ...
            remaining_accounts=[AccountMeta(transfer_authority_keypair.public_key, True, False)],
            pre_instructions=[approval_instruction],
            signers=[user_key, mint_key, transfer_authority_keypair]
        )
    )
    return result
